...
export class CamoufoxPlugin extends PluginBase implements BrowserAdapter, ToolProvider, ContextProvider {
  ...
  async stop(): Promise<void> {
    if (!this.isRunning) return
    try { await this.rpc('shutdown') } catch { /* may be gone */ }
  ...
  async navigate(url: string, opts?: NavigateOptions): Promise<PageContent> {
    ...
    return (await this.rpc('navigate', {
      url, wait_until: opts?.waitUntil ?? 'domcontentloaded',
  ...
  async screenshot(opts?: ScreenshotOptions): Promise<ScreenshotData> {
    ...
    return (await this.rpc('screenshot', {
      full_page: opts?.fullPage ?? false, selector: opts?.selector,
    })) as ScreenshotData
  }

  async click(selector: string): Promise<void> { await this.rpc('click', { selector }) }
  async type(selector: string, text: string): Promise<void> { await this.rpc('type', { selector, text }) }
  ...
  async getUrl(): Promise<string> { return (await this.rpc('get_url')) as string }
  ...
  private buildTools(prefix: string): Tool[] {
    ...
    return [
      ...
      buildTool({
        ...
        async call(input: Record<string, unknown>) {
          return { data: await browser.screenshot({
      ...
      buildTool({
        ...
        describe: (input) => `Extract elements matching ${(input as Record<string, unknown>).selector}`,
        async call(input: Record<string, unknown>) {
  ...
  async back(): Promise<void> { await this.rpc('back') }
  async forward(): Promise<void> { await this.rpc('forward') }
  async reload(): Promise<void> { await this.rpc('reload') }

  async getTextContent(): Promise<string> { return (await this.rpc('get_text_content')) as string }
  async getHtml(): Promise<string> { return (await this.rpc('get_html')) as string }
  async getTitle(): Promise<string> { return (await this.rpc('get_title')) as string }

  async queryElements(selector: string): Promise<ElementInfo[]> {
    return (await this.rpc('query_elements', { selector })) as ElementInfo[]
  }

  async hover(selector: string): Promise<void> { await this.rpc('hover', { selector }) }
  async select(selector: string, value: string): Promise<void> {
    await this.rpc('select', { selector, value })
  }

  async getCookies(): Promise<Array<{ name: string; value: string; domain: string }>> {
    return (await this.rpc('get_cookies')) as Array<{ name: string; value: string; domain: string }>
  }
