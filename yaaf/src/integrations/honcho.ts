...
export class HonchoSession {
  ...
  async getRepresentation(peerId: string, signal?: AbortSignal): Promise<HonchoRepresentation> {
    const result = (await honchoFetch(
      this.config,
...
export class HonchoPlugin extends PluginBase implements MemoryAdapter, ContextProvider {
  ...
  override async initialize(): Promise<void> {
    if (this.config.defaultPeerId) {
      try {
        await this.registerPeer(this.config.defaultPeerId);
      } catch {
        /* may exist */
      }
    }
  ...
  async save(entry: MemoryEntry): Promise<string> {
    ...
    const sessionId =
      (entry.metadata?.sessionId as string) ?? this.config.defaultSessionId ?? "default";
    const session = this.getSession(sessionId, [peerId]);
    const result = await session.addMessage(
      peerId,
      "user",
      [`[Memory: ${entry.type}] ${entry.name}`, entry.description, "---", entry.content].join("\n"),
    );
  ...
  async getIndex(): Promise<string> {
    ...
    try {
      return (
        (await this.getPeerContext(peerId, { tokens: this.config.contextTokens })) ||
        "No memory index available."
      );
    } catch {
  ...
  async getContextSections(
    ...
    try {
      ...
      if (context) {
        return [
          { key: "honcho_user_context", content: context, placement: "system", priority: 15 },
        ];
      }
    } catch {
      /* non-fatal */
  ...
  async createSession(
    ...
    await honchoFetch(this.config, `/workspaces/${this.config.workspaceId}/sessions`, {
      method: "POST",
      body: { session_id: sessionId, peer_ids: [peerId], metadata },
      signal,
    });
