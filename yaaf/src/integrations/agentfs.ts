...
export class AgentFSPlugin extends PluginBase implements FileSystemAdapter, ToolProvider, ContextProvider {
  override readonly capabilities: readonly PluginCapability[] = [
    'filesystem', 'tool_provider', 'context_provider',
  ]
  ...
  private resolve(path: string): {
    node: FSNode | undefined; parent: FSNode | undefined; name: string
  } {
  ...
  async write(path: string, content: string, agentId?: string): Promise<void> {
    ...
    if (this.totalSize + contentSize > this.maxTotalSize) {
      throw new Error(`Storage quota exceeded: ${this.totalSize + contentSize} > ${this.maxTotalSize}`)
    }
    ...
    if (this.trackChangesEnabled) {
      this.changes.push({
        ...
        path: normalized, timestamp: Date.now(), agentId,
      })
    }
  }
  ...
  async remove(path: string, agentId?: string): Promise<boolean> {
    ...
    if (this.trackChangesEnabled) {
      this.changes.push({ type: 'delete', path: normalized, timestamp: Date.now(), agentId })
    }
    return true
  }
  ...
  private buildTree(node: FSNode, parentPath: string, depth: number, maxDepth: number): TreeEntry[] {
    ...
    for (const [name, child] of node.children) {
      ...
      if (child.type === 'directory') entry.children = this.buildTree(child, fullPath, depth + 1, maxDepth)
      entries.push(entry)
    }
    return entries.sort((a, b) => a.name.localeCompare(b.name))
  }
  ...
  discoverTools(basePath: string = '/tools'): Array<{
    path: string; name: string; description: string; isReadOnly: boolean
  }> {
    ...
    for (const entry of entries) {
      if (entry.type === 'tool') {
        ...
        if (tool) {
          tools.push({
            path: entry.path, name: tool.name,
            description: tool.userFacingName(undefined), isReadOnly: tool.isReadOnly({}),
          })
        }
      }
    }
