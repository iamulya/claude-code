...
const ANTHROPIC_PRICING: Record<string, LLMPricing> = {
  "claude-opus-4": { inputPerMillion: 15.0, outputPerMillion: 75.0, cacheReadPerMillion: 1.5 },
  "claude-sonnet-4": { inputPerMillion: 3.0, outputPerMillion: 15.0, cacheReadPerMillion: 0.3 },
  "claude-3-7-sonnet": { inputPerMillion: 3.0, outputPerMillion: 15.0, cacheReadPerMillion: 0.3 },
  "claude-3-5-sonnet": { inputPerMillion: 3.0, outputPerMillion: 15.0, cacheReadPerMillion: 0.3 },
  "claude-3-5-haiku": { inputPerMillion: 0.8, outputPerMillion: 4.0, cacheReadPerMillion: 0.08 },
  "claude-3-opus": { inputPerMillion: 15.0, outputPerMillion: 75.0, cacheReadPerMillion: 1.5 },
  "claude-3-sonnet": { inputPerMillion: 3.0, outputPerMillion: 15.0, cacheReadPerMillion: 0.3 },
...
export class AnthropicChatModel extends BaseLLMAdapter implements StreamingChatModel {
  ...
  async *stream(params: {
    ...
    let buffer = "";
    /** Cap SSE buffer to prevent OOM on malformed/stalled server responses */
    ...
    try {
      while (true) {
        // Check abort at the top of every read() iteration.
        // The AbortSignal is wired to doFetch() terminating the network request,
        // but we also guard here in case the SDK/provider continues delivering data.
        ...
        // Reject responses where the buffer grows without hitting an
        // event boundary (µµ separator). A stalled or adversarial proxy could
        // send bytes continuously without ever completing an event.
        if (buffer.length > MAX_SSE_BUFFER_BYTES) {
          throw new Error(
            `[yaaf/anthropic] SSE buffer exceeded ${MAX_SSE_BUFFER_BYTES} bytes without a complete event. ` +
