...
export class CostTracker {
  ...
  formatSummary(): string {
    ...
    lines.push(
      `Total cost: ${CostTracker.formatCost(this._totalCostUSD)}${this._hasUnknownCost ? " (some models have unknown pricing)" : ""}`,
    );
    lines.push(
      `Total tokens: ${CostTracker.formatNumber(this._totalInputTokens)} in, ${CostTracker.formatNumber(this._totalOutputTokens)} out`,
    );
  ...
  static restore(
    ...
    for (const [model, usage] of Object.entries(snapshot.models)) {
      // Validate per-model usage fields before storing.
      // A corrupted session file could produce NaN or negative values that
      // silently break guardrail cost checks (NaN >= limit is always false).
      const safeUsage: ModelUsage = {
        inputTokens: safeNonNegativeInt(usage.inputTokens),
        outputTokens: safeNonNegativeInt(usage.outputTokens),
        cacheReadTokens: safeNonNegativeInt(usage.cacheReadTokens),
        cacheWriteTokens: safeNonNegativeInt(usage.cacheWriteTokens),
        costUSD: safeNonNegativeFloat(usage.costUSD),
        calls: safeNonNegativeInt(usage.calls),
      };
      tracker.models.set(model, safeUsage);
    }
    tracker._totalCostUSD = safeNonNegativeFloat(snapshot.totalCostUSD);
    tracker._totalInputTokens = safeNonNegativeInt(snapshot.totalInputTokens);
    tracker._totalOutputTokens = safeNonNegativeInt(snapshot.totalOutputTokens);
    return tracker;
  }
}

// W10-04 helpers: clamp to safe numeric values
function safeNonNegativeFloat(v: unknown): number {
  const n = typeof v === "number" ? v : Number(v);
  return isFinite(n) && n >= 0 ? n : 0;
}
function safeNonNegativeInt(v: unknown): number {
  return Math.floor(safeNonNegativeFloat(v));
}
