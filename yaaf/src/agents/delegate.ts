...
export class AgentRouter {
   ...
   *
   * Encode channelName and senderId with encodeURIComponent() before
   * embedding them in the key. Without this, a channelName or senderId containing
   * ':' causes key collisions — e.g., channelName='a:b', senderId='c' and
   * channelName='a', senderId='b:c' both produce the same key.
   */
  resolveSessionKey(agentId: string, message: RoutableMessage): string {
    const entry = this.agents.get(agentId);
    const scope = entry?.sessionScope ?? "per-channel-sender";
    // Safe encoding: encodeURIComponent produces chars from [A-Za-z0-9_.!~*'()-]
    // which cannot contain ':' or '@', so our delimiters are unambiguous.
    const safeChannel = encodeURIComponent(message.channelName ?? "unknown");
    const safeSender = encodeURIComponent(message.senderId ?? "unknown");

    switch (scope) {
      case "shared":
        return `${agentId}:main`;
      case "per-sender":
        return `${agentId}:s:${safeSender}`;
      case "per-channel":
        return `${agentId}:c:${safeChannel}`;
      case "per-channel-sender":
        return `${agentId}:${safeChannel}:${safeSender}`;
    }
  }
