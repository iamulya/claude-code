        ...
        switch (msg.type) {
          ...
          case 'resume': {
            ...
            if (session) {
              ...
              ws.send(JSON.stringify({ type: 'error', sessionId: msg.sessionId, message: 'Session not found' }))
            }
            break
          }
            ...
            try {
              ...
              if (controller.signal.aborted) {
                this.broadcast(session, {
                  type: 'error', sessionId: session.id, message: 'Canceled',
                })
              } else {
                this.broadcast(session, {
                  type: 'error', sessionId: session.id,
