...
export class SummarizeStrategy implements CompactionStrategy {
  ...
  async compact(ctx: CompactionContext): Promise<StrategyResult | null> {
    ...
    if (!summary) throw new Error('Summarization returned empty result')
    ...
    const postTokens = newMessages.reduce(
      (sum, m) => sum + ctx.estimateTokens(typeof m.content === 'string' ? m.content : JSON.stringify(m.content)),
      0,
    )
...
export class TruncateStrategy implements CompactionStrategy {
  ...
  async compact(ctx: CompactionContext): Promise<StrategyResult | null> {
    ...
    const cutPoint = Math.max(1, Math.floor(ctx.messages.length * this.dropRatio))
    ...
    for (const strategy of this.strategies) {
      ...
      if (!accumulatedResult) {
        ...
        accumulatedResult = {
          ...
          metadata: {
            strategies: [
              ...((accumulatedResult.metadata?.strategies as string[]) ?? [accumulatedResult.metadata?.strategy]),
