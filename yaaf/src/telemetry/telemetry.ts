...
async function buildOtlpTraceExporter(): Promise<AnySpanExporter> {
  ...
  switch (protocol) {
    case 'grpc': {
      const { OTLPTraceExporter } = await import('@opentelemetry/exporter-trace-otlp-grpc' as string)
    ...
    case 'http/json': {
      ...
      return new (OTLPTraceExporter as new (c: object) => AnySpanExporter)(
        { url: endpoint ? `${endpoint}/v1/traces` : undefined, headers },
      )
    }
    case 'http/protobuf':
    default: {
      const { OTLPTraceExporter } = await import('@opentelemetry/exporter-trace-otlp-proto' as string)
      return new (OTLPTraceExporter as new (c: object) => AnySpanExporter)(
        { url: endpoint ? `${endpoint}/v1/traces` : undefined, headers },
      )
    }
...
async function buildMetricReaders(): Promise<PeriodicExportingMetricReader[]> {
  ...
  for (const t of types) {
    if (t === 'console') {
      ...
      readers.push(new PeriodicExportingMetricReader({
        exporter:             exporter as ConstructorParameters<typeof PeriodicExportingMetricReader>[0]['exporter'],
        exportIntervalMillis: interval,
      }))
    } else {
...
async function buildOtlpMetricExporter() {
  ...
  switch (protocol) {
    case 'grpc': {
      const { OTLPMetricExporter } = await import('@opentelemetry/exporter-metrics-otlp-grpc' as string)
    ...
    case 'http/json': {
      ...
      return new (OTLPMetricExporter as new (c: object) => object)(
        { url: endpoint ? `${endpoint}/v1/metrics` : undefined, headers },
      )
    }
    case 'http/protobuf':
    default: {
      const { OTLPMetricExporter } = await import('@opentelemetry/exporter-metrics-otlp-proto' as string)
      return new (OTLPMetricExporter as new (c: object) => object)(
        { url: endpoint ? `${endpoint}/v1/metrics` : undefined, headers },
      )
    }
  ...
  switch (protocol) {
    ...
    case 'http/json': {
      ...
      return new (OTLPLogExporter as new (c: object) => AnyLogExporter)(
        { url: endpoint ? `${endpoint}/v1/logs` : undefined, headers },
      )
    }
    case 'http/protobuf':
    default: {
...
function telemetryTimeout(ms: number): Promise<never> {
  return new Promise((_, reject) => {
    const t = setTimeout(
      () => reject(new TelemetryTimeoutError(
        `YAAF telemetry flush timed out after ${ms}ms`,
      )),
      ms,
    )
...
export async function initYAAFTelemetry(): Promise<ReturnType<MeterProvider['getMeter']>> {
  ...
  process.on('beforeExit', () => { void shutdownFn() })
  process.on('exit', () => {
...
export async function flushYAAFTelemetry(): Promise<void> {
  ...
  try {
    ...
    log.warn(`Telemetry flush timed out after ${timeoutMs}ms — some data may not have been exported`)
  }
}
