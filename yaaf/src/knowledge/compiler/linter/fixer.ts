...
export async function applyFixes(
  ...
  for (const [docId, docIssues] of byDocId) {
    ...
    for (const issue of docIssues) {
      ...
      if (!modified.includes(findText)) {
        skipped.push({ issue, reason: `Target text "${findText.slice(0, 40)}" not found (may already be fixed)` })
        continue
      }
  ...
  for (const issue of nonFixable) {
    skipped.push({ issue, reason: 'Issue is not auto-fixable — requires human review or re-compilation' })
  }
