        ...
        if (entryStats.isDirectory()) {
          ...
          try {
            ...
            if (!ontology.entityTypes[entityType]) {
              warnings.push(`${docId}: entity_type "${entityType}" not in ontology — skipped`);
              return;
            }
            ...
            warnings.push(
              `${docId}: failed to read — ${err instanceof Error ? err.message : String(err)}`,
            );
          }
        }
      }),
 ...
 */
export function upsertRegistryEntry(registry: ConceptRegistry, entry: ConceptRegistryEntry): void {
 ...
 */
export function removeRegistryEntry(registry: ConceptRegistry, docId: string): boolean {
...
export function serializeRegistry(registry: ConceptRegistry): string {
  return JSON.stringify(Array.from(registry.entries()), null, 2);
}
