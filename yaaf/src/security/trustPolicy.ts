...
export class TrustPolicy {
  ...
  verifyPlugin(
    ...
    // Unknown plugin
    if (!trust) {
      const event = this.createEvent('plugin', pluginName,
  ...
  filterMcpTools(
    ...
    if (!trust) {
      if (this.unknownPolicy === 'deny') {
        const event = this.createEvent('mcp_server', serverName, 'blocked',
      ...
      const event = this.createEvent('mcp_server', serverName, 'unknown',
    ...
    if (trust.trusted) {
      const event = this.createEvent('mcp_server', serverName, 'trusted', 'Server is explicitly trusted')
      events.push(event)
      return { allowed: tools, blocked: [], events }
    }
    ...
    for (const tool of tools) {
      ...
      if (isAllowed) {
        allowed.push(tool)
        events.push(this.createEvent('mcp_tool', `${serverName}/${tool.name}`, 'verified',
          `Tool "${tool.name}" allowed by trust policy`))
      } else {
        blocked.push(tool.name)
        events.push(this.createEvent('mcp_tool', `${serverName}/${tool.name}`, 'blocked',
