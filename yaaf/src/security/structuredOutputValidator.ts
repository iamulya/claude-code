...
export class StructuredOutputValidator {
  ...
  validate(output: string): OutputValidationEvent {
    ...
    if (violations.length > 0) {
      action = this.onViolation === 'reject' ? 'rejected' :
        this.onViolation === 'strip' ? 'stripped' : 'warned'
    }
      ...
      switch (rule.type) {
        case 'string':
          if (typeof value !== 'string') {
            violations.push({ field: rule.field, rule: 'type', actual: typeof value, expected: 'string' })
          } else if (rule.maxLength && value.length > rule.maxLength) {
            violations.push({ field: rule.field, rule: 'maxLength', actual: value.length, expected: `<= ${rule.maxLength}` })
          } else if (rule.pattern && !rule.pattern.test(value)) {
            violations.push({ field: rule.field, rule: 'pattern', actual: value, expected: rule.pattern.source })
          }
          break

        case 'number':
          if (typeof value !== 'number') {
            violations.push({ field: rule.field, rule: 'type', actual: typeof value, expected: 'number' })
          } else {
            if (rule.min !== undefined && value < rule.min) {
              violations.push({ field: rule.field, rule: 'min', actual: value, expected: `>= ${rule.min}` })
            }
            if (rule.max !== undefined && value > rule.max) {
              violations.push({ field: rule.field, rule: 'max', actual: value, expected: `<= ${rule.max}` })
            }
          }
          break
        ...
        case 'boolean':
          if (typeof value !== 'boolean') {
            violations.push({ field: rule.field, rule: 'type', actual: typeof value, expected: 'boolean' })
          }
          break

        case 'url':
          if (typeof value !== 'string' || !isValidUrl(value)) {
            violations.push({ field: rule.field, rule: 'type', actual: value, expected: 'valid URL' })
          }
          break

        case 'email':
          if (typeof value !== 'string' || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)) {
            violations.push({ field: rule.field, rule: 'type', actual: value, expected: 'valid email' })
          }
          break
...
export function structuredOutputValidator(config?: OutputValidatorConfig): StructuredOutputValidator {
