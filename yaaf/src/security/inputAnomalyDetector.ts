  ...
  analyze(messages: ChatMessage[]): InputAnomalyResult {
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      if (content.length > this.hardMaxInputLength) {
        const event = this.createEvent('length_blocked', i, msg.role, 'block',
      ...
      if (content.length > this.maxInputLength) {
        events.push(this.createEvent('length_warning', i, msg.role, 'warning',
      ...
      if (content.length > 100) {
        ...
        if (entropy < this.minEntropy) {
          events.push(this.createEvent('low_entropy', i, msg.role, 'warning',
            `Low entropy (${entropy.toFixed(2)} bits) suggests repetitive padding attack`))
        }

        if (entropy > this.maxEntropy) {
          events.push(this.createEvent('high_entropy', i, msg.role, 'warning',
      ...
      if (invisibleRatio > this.maxInvisibleRatio) {
        events.push(this.createEvent('invisible_chars', i, msg.role, 'warning',
      ...
      if (content.length > 200) {
        ...
        if (repRatio > this.maxRepetitionRatio) {
          events.push(this.createEvent('repetition', i, msg.role, 'warning',
      ...
      if (content.length > 20) {
        ...
        if (scripts.size >= 3) {
          events.push(this.createEvent('mixed_scripts', i, msg.role, 'warning',
