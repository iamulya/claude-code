...
describe('KBCompiler.create()', () => {
  ...
  it('starts with an empty registry when no .kb-registry.json exists', async () => {
    ...
    const compiler = await KBCompiler.create({ kbDir, extractionModel: extractFn, synthesisModel: synthFn })
...
describe('KBCompiler.compile()', () => {
  ...
  it('dry run does not write any files', async () => {
    ...
    try { await stat(expectedPath); fileExists = true } catch { /* expected */ }
