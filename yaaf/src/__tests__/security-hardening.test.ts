...
describe("InputAnomalyDetector", () => {
  ...
  describe("entropy analysis", () => {
    ...
    it("passes normal English text", () => {
      const detector = new InputAnomalyDetector();
      const text =
        "The quick brown fox jumps over the lazy dog. This is a normal English sentence with reasonable entropy. Nothing suspicious here at all, just regular conversational text about everyday topics.";
      const result = detector.analyze([userMsg(text)]);
      expect(
        result.events.filter((e) => e.type === "low_entropy" || e.type === "high_entropy"),
      ).toHaveLength(0);
    });
  });
...
describe("StructuredOutputValidator", () => {
  ...
  describe("field validation", () => {
    ...
    it("validates string maxLength", () => {
      ...
      expect(
        validator.validate('{"bio": "Very long biography text"}').violations.length,
      ).toBeGreaterThan(0);
    });
  });
  ...
  describe("URL validation", () => {
    it("validates URLs in text", () => {
      ...
      const result = validator.validate(
        "Check out https://evil.com/phishing and https://example.com/page",
      );
...
describe("SecurityAuditLog", () => {
  ...
  describe("integration callbacks", () => {
    it("creates middleware callback functions", () => {
      ...
      callbacks.piiRedactor({
        category: "email",
        direction: "input",
        count: 2,
        action: "redacted",
      });
      callbacks.rateLimiter({
        userId: "user-1",
        resource: "cost",
        action: "blocked",
        current: 10,
