...
import { verifyJwt, decodeJwt, JwtError } from "../iam/jwt.js";
...
describe("JWT verification", () => {
  ...
  it("rejects an expired token", async () => {
    ...
    await expect(verifyJwt(token, TEST_SECRET, { algorithms: ["HS256"] })).rejects.toThrow(
      "JWT expired",
    );
  });
  ...
  it("rejects a tampered token", async () => {
    ...
    await expect(verifyJwt(tampered, TEST_SECRET, { algorithms: ["HS256"] })).rejects.toThrow(
  ...
  it("rejects a wrong secret", async () => {
    ...
    await expect(verifyJwt(token, "wrong-secret", { algorithms: ["HS256"] })).rejects.toThrow(
      "Invalid JWT signature",
    );
  });
  ...
  it("rejects disallowed algorithms", async () => {
    ...
    await expect(verifyJwt(token, TEST_SECRET, { algorithms: ["RS256"] })).rejects.toThrow(
      'Algorithm "HS256" not allowed',
    );
  });
...
describe("JwtIdentityProvider", () => {
  ...
  it("resolves identity from Bearer token", async () => {
    ...
    const user = await provider.resolve(makeRequest({ authorization: `Bearer ${token}` }));
  ...
  it("throws for invalid token (X-5: fail-closed)", async () => {
    await expect(
      provider.resolve(makeRequest({ authorization: "Bearer invalid.token.here" })),
    ).rejects.toThrow();
  });
  ...
  it("throws for expired token (X-5: fail-closed)", async () => {
    const token = createTestJwt({ sub: "user-1", exp: now - 3600 });
    await expect(
      provider.resolve(makeRequest({ authorization: `Bearer ${token}` })),
  ...
  it("reads from query parameter when configured", async () => {
    ...
    const user = await provider2.resolve(makeRequest({}, { token }));
  ...
  it("supports nested claim paths", async () => {
    ...
    const user = await provider3.resolve(makeRequest({ authorization: `Bearer ${token}` }));
  ...
  it("rejects tokens with wrong issuer", async () => {
    ...
    await expect(
      strictProvider.resolve(makeRequest({ authorization: `Bearer ${token}` })),
    ).rejects.toThrow("Issuer mismatch");
  });
...
describe("ApiKeyIdentityProvider", () => {
  it("resolves from static key map", async () => {
    ...
    const user = await provider.resolve(makeRequest({ authorization: "Bearer sk-prod-abc" }));
  ...
  it("resolves from X-API-Key header", async () => {
    ...
    const user = await provider.resolve(makeRequest({ "x-api-key": "my-key" }));
  ...
  it("resolves from dynamic resolver", async () => {
    ...
    const user = await provider.resolve(makeRequest({ authorization: "Bearer dynamic-key-123" }));
  ...
  it("tries static first, then dynamic", async () => {
    ...
    const user1 = await provider.resolve(makeRequest({ authorization: "Bearer known-key" }));
    expect(user1!.userId).toBe("static-user");
    expect(resolver).not.toHaveBeenCalled();

    // Unknown key — falls through to resolver
    const user2 = await provider.resolve(makeRequest({ authorization: "Bearer unknown-key" }));
  ...
  it("returns null for unknown keys", async () => {
    ...
    const user = await provider.resolve(makeRequest({ authorization: "Bearer invalid-key" }));
    expect(user).toBeNull();
  });
  ...
  it("reads from custom header", async () => {
    ...
    const user = await provider.resolve(makeRequest({ "x-custom-auth": "custom-key" }));
...
describe("CompositeIdentityProvider", () => {
  it("tries providers in order — first match wins", async () => {
    ...
    const user1 = await composite.resolve(makeRequest({ authorization: `Bearer ${token}` }));
    expect(user1!.userId).toBe("jwt-user");

    // API key is used when JWT fails
    const user2 = await composite.resolve(makeRequest({ "x-api-key": "fallback-key" }));
  ...
  it("collects errors from providers but still tries fallback (X-6)", async () => {
    ...
    const user = await composite.resolve(makeRequest({ "x-api-key": "key" }));
