  ...
  it('missing toolCallId falls back to empty string (not undefined)', async () => {
    ...
    await m.complete({
      messages: [
        ...
        { role: 'assistant', content: 'using tool', toolCalls: [{ id: 'c1', name: 't', arguments: '{}' }] },
...
describe('Anthropic — malformed tool arguments (documented behavior)', () => {
  ...
  it('unparseable arguments silently become {}', async () => {
    ...
    await m.complete({
      messages: [
        ...
        { role: 'assistant', content: '', toolCalls: [
          { id: 'c1', name: 'search', arguments: 'THIS IS NOT JSON' },
        ]},
      ],
    })
    ...
    const toolUseBlock = (Array.isArray(assistantMsg.content) ? assistantMsg.content : [])
