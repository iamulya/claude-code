...
describe('GroundingValidator', () => {
  describe('assessment', () => {
    ...
    it('returns 1.0 score when no tool evidence exists', () => {
      const assessment = validator.assess(
        'Just a normal response without any tool data.',
        [userMsg('Hello')],
      )
    ...
    it('skips short sentences (greetings, acks)', () => {
      const messages: ChatMessage[] = [
        toolMsg('Data here.', 'api'),
      ]

      const assessment = validator.assess('Sure! Here you go.', messages)
  ...
  describe('annotate mode', () => {
    it('adds ungrounded markers', () => {
      ...
      const messages: ChatMessage[] = [
        toolMsg('The database contains 1000 users.', 'db_query'),
      ]
  ...
  describe('strict mode', () => {
    it('marks response as overridden when below threshold', () => {
      const validator = new GroundingValidator({
        mode: 'strict',
        minCoverage: 0.9,
        minOverlapTokens: 3,
  ...
  describe('hooks integration', () => {
    it('creates working afterLLM hook', () => {
      ...
      const result = afterLLM(response, 0)
      // With no messages captured, should pass
      expect(result).toEqual({ action: 'continue' })
    })

    it('beforeLLM captures messages for grounding', () => {
      const validator = groundingValidator({ mode: 'warn' })
      ...
      beforeLLM([
        userMsg('Query'),
        toolMsg('Some tool evidence data.', 'search'),
      ])
  ...
  describe('event callbacks', () => {
    it('fires onAssessment', () => {
      const scores: number[] = []
      const validator = new GroundingValidator({
        onAssessment: (e) => scores.push(e.score),
      })

      validator.assess('Hello.', [])
...
describe('PerUserRateLimiter', () => {
  ...
  describe('cost limits', () => {
    it('allows users within their cost budget', () => {
      limiter = new PerUserRateLimiter({ maxCostPerUser: 5.00, gcIntervalMs: 0 })

      limiter.recordUsage('alice', { cost: 2.00 })
      const check = limiter.check('alice')
      expect(check.blocked).toBe(false)
      expect(check.usage.cost).toBe(2.00)
    })

    it('blocks users who exceed cost budget', () => {
      limiter = new PerUserRateLimiter({ maxCostPerUser: 5.00, gcIntervalMs: 0 })

      limiter.recordUsage('alice', { cost: 3.00 })
      limiter.recordUsage('alice', { cost: 2.50 })
  ...
  describe('role bypass', () => {
    ...
    it('enforces limits for non-bypass roles', () => {
      ...
      limiter.recordUsage('regular-user', { cost: 2.00 })
  ...
  describe('enforce method', () => {
    it('throws on limit violation', () => {
      limiter = new PerUserRateLimiter({ maxCostPerUser: 1.00, gcIntervalMs: 0 })
      limiter.recordUsage('user', { cost: 2.00 })

      expect(() => limiter.enforce('user')).toThrow('cost limit exceeded')
    })

    it('does not throw within limits', () => {
      limiter = new PerUserRateLimiter({ maxCostPerUser: 10.00, gcIntervalMs: 0 })
  ...
  describe('user management', () => {
    it('tracks multiple users independently', () => {
      ...
      limiter.recordUsage('alice', { cost: 4.00 })
      limiter.recordUsage('bob', { cost: 1.00 })

      expect(limiter.getUsage('alice').cost).toBe(4.00)
      expect(limiter.getUsage('bob').cost).toBe(1.00)
    })
