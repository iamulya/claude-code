...
describe('checkInvalidFieldValues', () => {
  ...
  it('returns issue for invalid enum value', () => {
    ...
    const issues = checkInvalidFieldValues(article, TEST_ONTOLOGY)
    expect(issues.some(i => i.code === 'INVALID_FIELD_VALUE' && i.field?.includes('status'))).toBe(true)
  })
})
...
describe('checkDuplicateCandidates', () => {
  it('detects very similar titles', () => {
    const articles = [
      makeArticle({ docId: 'concepts/a', frontmatter: { title: 'Attention Mechanism', entity_type: 'concept' } }),
      makeArticle({ docId: 'concepts/b', frontmatter: { title: 'Attention Mechanisms', entity_type: 'concept' } }),
    ]
  ...
  it('does not flag clearly different titles', () => {
    const articles = [
      makeArticle({ docId: 'concepts/a', frontmatter: { title: 'Transformer', entity_type: 'concept' } }),
      makeArticle({ docId: 'concepts/b', frontmatter: { title: 'Recurrent Neural Network', entity_type: 'concept' } }),
    ]
    const issues = checkDuplicateCandidates(articles, 0.15)
    expect(issues).toHaveLength(0)
  })
...
describe('applyFixes', () => {
  ...
  it('fixes NON_CANONICAL_WIKILINK in the file', async () => {
    ...
    const issue = {
      ...
      docId: 'concepts/transformer',
      autoFixable: true,
      fix: { findText: '[[self-attention]]', replaceWith: '[[Attention Mechanism]]', firstOccurrenceOnly: false },
    }
...
describe('KBLinter integration', () => {
  ...
  it('produces a report with correct article count', async () => {
    await writeArticle('concepts/transformer', [
      '---',
      'title: "Transformer"',
      'entity_type: concept',
      'stub: false',
    ...
    await writeArticle('concepts/attention-mechanism', [
      '---',
