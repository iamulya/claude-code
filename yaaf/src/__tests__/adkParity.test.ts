...
describe('parallel', () => {
  ...
  it('fail-fast mode propagates errors', async () => {
    const fanOut = parallel(
      [labelStep('A'), failStep('boom'), labelStep('C')],
      { onError: 'fail-fast' },
    )
  ...
  it('bounded concurrency works', async () => {
    ...
    const fanOut = parallel(
      [makeTimedStep('A'), makeTimedStep('B'), makeTimedStep('C')],
      { concurrency: 2 },
    )
...
describe('loop', () => {
  ...
  it('runs multiple steps per iteration', async () => {
    const loopAgent = loop(
      [labelStep('eval'), labelStep('improve')],
      { maxIterations: 2 },
    )
  ...
  it('onIteration transforms between iterations', async () => {
    let iteration = 0
    const loopAgent = loop(
      [mockStep(input => input.toUpperCase())],
      {
        maxIterations: 2,
