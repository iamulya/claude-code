...
/**
 * Spy on AgentRunner.setSystemOverride for the duration of fn(), collect all
 * injected strings, then restore the original method.
 */
async function captureOverrides(fn: () => Promise<void>): Promise<string[]> {
  const { AgentRunner } = await import('../agents/runner.js')
  const captured: string[] = []
  const orig = AgentRunner.prototype.setSystemOverride
  AgentRunner.prototype.setSystemOverride = function(o: string) {
    captured.push(o)
  ...
  try {
    ...
    AgentRunner.prototype.setSystemOverride = orig
  }
  return captured
}

// ════════════════════════════════════════════════════════════════════════════
// GAP 1 — MemoryAdapter is consulted as path 3 in buildMemoryPrefix()
// ════════════════════════════════════════════════════════════════════════════
...
describe('GAP 1 — MemoryAdapter plugin wired into Agent.buildMemoryPrefix()', () => {
  ...
  it('MemoryAdapter.search() is called with the user message as the query', async () => {
    ...
    const searchFn = vi.fn().mockResolvedValue([
      {
        entry: {
          id: 'm0', content: 'User prefers TypeScript', description: 'User prefers TypeScript',
          type: 'fact', tags: [], source: 'test',
    ...
    const memAdapter: MemoryAdapter = {
      ...
      async initialize() {},
      async destroy() {},
      async save() { return 'mem-id' },
      async delete() {},
      search: searchFn,
      buildPrompt() { return 'memo' },
    }
  ...
  it('MemoryAdapter search results appear in the runner system prompt override', async () => {
    ...
    const memAdapter: MemoryAdapter = {
      ...
      async initialize() {},
      async destroy() {},
      async save() { return 'id' },
      async delete() {},
      async search() {
        return [{
          entry: {
            id: 'm1', content: 'User loves TypeScript', description: 'User loves TypeScript',
            type: 'fact', tags: [], source: 'test',
            createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(),
          } as MemoryEntry,
          score: 0.9,
      ...
      buildPrompt() { return 'User loves TypeScript' },
    }
  ...
  it('MemoryAdapter.buildPrompt() is used as fallback when search() returns empty', async () => {
    ...
    const memAdapter: MemoryAdapter = {
      ...
      async delete() {},
      async search() { return [] },
      buildPrompt() { return 'Global memory: user is a senior engineer.' },
    }
  ...
  it('MemoryAdapter errors are swallowed silently and do not crash the agent', async () => {
    ...
    const memAdapter: MemoryAdapter = {
      ...
      async save() { return 'id' },
      async delete() {},
      async search() { throw new Error('DB connection lost') },
      buildPrompt() { throw new Error('DB connection lost') },
    }
...
// ════════════════════════════════════════════════════════════════════════════
// GAP 2 — ContextProvider.gatherContext() called per Agent.run() turn
// ════════════════════════════════════════════════════════════════════════════
...
describe('GAP 2 — ContextProvider plugin gatherContext() called in Agent.run()', () => {
  ...
  it('multiple ContextProviders are all consulted and their sections are merged', async () => {
    ...
    const getSectionsA = vi.fn().mockResolvedValue([
      { key: 'ctx-a', content: 'Context from A', placement: 'system' as const },
    ] as ContextSection[])
    const getSectionsB = vi.fn().mockResolvedValue([
      { key: 'ctx-b', content: 'Context from B', placement: 'system' as const },
    ] as ContextSection[])

    const providerA: ContextProvider = {
      name: 'provider-a', version: '1.0.0', capabilities: ['context_provider'] as const,
      async initialize() {}, async destroy() {}, getContextSections: getSectionsA,
    }
    const providerB: ContextProvider = {
      name: 'provider-b', version: '1.0.0', capabilities: ['context_provider'] as const,
      async initialize() {}, async destroy() {}, getContextSections: getSectionsB,
    }
  ...
  it('ContextProvider errors are swallowed and do not crash Agent.run()', async () => {
    ...
    const provider: ContextProvider = {
      name: 'crashing-ctx', version: '1.0.0', capabilities: ['context_provider'] as const,
      async initialize() {}, async destroy() {},
      async getContextSections() { throw new Error('Provider exploded') },
    }
  ...
  it('getContextSections() is called on EACH turn independently (not cached)', async () => {
    ...
    const provider: ContextProvider = {
      name: 'per-turn-provider', version: '1.0.0', capabilities: ['context_provider'] as const,
      async initialize() {}, async destroy() {}, getContextSections: getSectionsFn,
    }
...
// ════════════════════════════════════════════════════════════════════════════
// GAP 3 — SessionAdapter auto-wired in Agent.create()
// ════════════════════════════════════════════════════════════════════════════
...
describe('GAP 3 — SessionAdapter plugin auto-wired in Agent.create()', () => {
  ...
  function makeSessionAdapter() {
    ...
    const deleteFn = vi.fn(async (id: string) => { store.delete(id) })
