  ...
  describe("RemoteSessionServer", () => {
    ...
    it("should provide health and info endpoints", async () => {
      ...
      try {
        ...
        // Sessions list — W4-03: endpoint now returns aggregate stats only
        // (no session IDs exposed to prevent hijacking attacks)
        const sessionsRes = await fetch(`${handle.url}/sessions`);
        expect(sessionsRes.ok).toBe(true);
        const sessions = (await sessionsRes.json()) as { count: number; maxSessions: number };
        expect(sessions.count).toBe(0);
        expect(sessions.maxSessions).toBeGreaterThan(0);
      } finally {
        await handle.close();
      }
    });
...
describe("MCP OAuth", () => {
  describe("McpOAuthClient", () => {
    ...
    it("should validate state on code exchange", async () => {
      ...
      await expect(client.exchangeCode("some-code", "wrong-state")).rejects.toThrow(
        "state mismatch",
      );
    });
