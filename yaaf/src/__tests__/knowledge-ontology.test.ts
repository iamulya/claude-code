    ...
    concept: {
      ...
      frontmatter: {
        fields: {
          ...
          entity_type: { description: 'Entity type', type: 'enum', required: true, enum: ['concept', 'tool'] },
...
describe('OntologyLoader', () => {
  ...
  it('reports error when domain is missing', async () => {
    const yaml = VALID_ONTOLOGY_YAML.replace('domain: "TypeScript agent framework documentation"', '')
