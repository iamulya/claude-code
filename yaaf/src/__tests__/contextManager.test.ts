...
function addMessages(ctx: ContextManager, msgs: Array<{ role: 'user' | 'assistant' | 'tool_result'; content: string; metadata?: Record<string, unknown> }>) {
  for (const m of msgs) {
    ctx.addMessage(m)
  }
}
...
describe('ContextManager — sections', () => {
  it('addSection and getSections', () => {
    ...
    ctx.addSection({ key: 'memory', content: 'user likes cats', placement: 'system', priority: 20 })
...
describe('ContextManager — messages', () => {
  ...
  it('addMessage preserves custom uuid and timestamp', () => {
    const ctx = makeCtx()
    const msg = ctx.addMessage({ role: 'user', content: 'Hello', uuid: 'custom-id', timestamp: 12345 })
...
describe('ContextManager — micro-compaction', () => {
  ...
  it('respects microCompactableTools filter', () => {
    ...
    ctx.addMessage({ role: 'tool_result', content: 'search data', metadata: { toolName: 'search' } })
    ctx.addMessage({ role: 'tool_result', content: 'calc data', metadata: { toolName: 'calc' } })
    ctx.addMessage({ role: 'tool_result', content: 'search latest', metadata: { toolName: 'search' } })
...
describe('ContextManager — full compaction', () => {
  ...
  it('compact with onExtractFacts failure does not block compaction', async () => {
    const ctx = new ContextManager({
      ...
      compactionStrategy: 'summarize',
      summarizeFn: async () => 'Summary.',
      onExtractFacts: async () => { throw new Error('extraction failed') },
    })
...
describe('ContextManager — plugin strategy', () => {
  it('delegates to strategy.compact when strategy plugin is set', async () => {
    const compactFn = vi.fn(async (ctx: any) => ({
      messages: [{ uuid: 'new', role: 'assistant' as const, content: 'Plugin summary', timestamp: Date.now() }],
