...
describe('RunnerEvents emission', () => {
  ...
  describe('iteration events', () => {
    it('emits iteration count on each loop', async () => {
      ...
      const runner = new AgentRunner({ model, tools: [echoTool], systemPrompt: 'Test', maxIterations: 10 })
  ...
  describe('context:output-continuation', () => {
    it('emits when finishReason is length (via runStream)', async () => {
      ...
      for await (const _event of runner.runStream('Write a long essay')) { /* drain */ }
  ...
  describe('usage events', () => {
    it('emits usage after each LLM call', async () => {
      const model = createMockModel([
        { content: 'Hello', finishReason: 'stop', usage: { promptTokens: 10, completionTokens: 5 } },
      ])
      const runner = new AgentRunner({ model, tools: [], systemPrompt: 'Test' })
...
describe('Hook event propagation', () => {
  it('emits hook:error when beforeToolCall throws', async () => {
    const hooks: Hooks = {
      beforeToolCall: async () => { throw new Error('Hook went boom') },
    }
    const model = createMockModel([
      {
        content: '',
  ...
  it('emits hook:error when afterToolCall throws', async () => {
    const hooks: Hooks = {
      afterToolCall: async () => { throw new Error('After hook crashed') },
    }
    const model = createMockModel([
      {
        content: '',
  ...
  it('emits hook:error when beforeLLM throws', async () => {
    const hooks: Hooks = {
      beforeLLM: async () => { throw new Error('LLM hook broken') },
    }
    const model = createMockModel([
  ...
  it('emits hook:error when afterLLM throws', async () => {
    const hooks: Hooks = {
      afterLLM: async () => { throw new Error('After LLM kaboom') },
    }
    const model = createMockModel([
      { content: 'Response', finishReason: 'stop' },
    ])
...
describe('Doctor watch integration', () => {
  ...
  it('watch() receives hook:error events as DoctorIssues', async () => {
    ...
    const agent = new Agent({
      ...
      tools: [echoTool],
      hooks: {
        beforeToolCall: async () => { throw new Error('Hook broke') },
      },
    })
  ...
  it('watch() receives context:output-continuation events', async () => {
    ...
    for await (const _event of agent.runStream('Write long')) { /* drain */ }
    await wait(100)
...
describe('Streaming events', () => {
  it('emits the same events via runStream as via run', async () => {
    ...
    // Consume the stream
    for await (const _event of runner.runStream('Test')) { /* drain */ }
  ...
  it('emits context:output-continuation via runStream', async () => {
    ...
    for await (const _event of runner.runStream('Write long')) { /* drain */ }
  ...
  it('emits hook:error via runStream when hooks throw', async () => {
    const hooks: Hooks = {
      beforeLLM: async () => { throw new Error('Stream hook error') },
    }
    const model = createMockModel([
      { content: 'Response', finishReason: 'stop' },
    ])
...
describe('Doctor diagnosis pipeline (with spy model)', () => {
  it('sends error context to Doctor LLM during diagnosis', async () => {
    ...
    doctor.onIssue((issue) => issues.push({
      type: issue.type,
    ...
    const diagnosisIssue = issues.find(i => i.summary.includes('diagnosis') || i.summary.includes('analyzed'))
  ...
  it('Doctor diagnosis output becomes a DoctorIssue', async () => {
    ...
    const diagnosisText = 'The system prompt may be too restrictive. Try adding an instruction like "Always respond to the user."'
  ...
  it('gracefully handles Doctor LLM failure', async () => {
    ...
    const failingDoctorModel: ChatModel & { model: string } = {
      ...
      async complete() { throw new Error('Doctor API key expired') },
    }
