 ...
 */
async function buildPluginHost(
  ...adapters: Array<ObservabilityAdapter | NotificationAdapter>
): Promise<PluginHost> {
...
describe("GAP 8 — Server /health reflects plugin health", () => {
  ...
  it("returns 200 with status:ok when all plugins healthy", async () => {
    const agent: ServerAgent = {
      run: async () => "ok",
      async healthCheck() {
        return { "redis-session": true, "honcho-memory": true };
      },
    };
  ...
  it("returns 503 with status:degraded when any plugin is unhealthy", async () => {
    const agent: ServerAgent = {
      run: async () => "ok",
      async healthCheck() {
        return { "redis-session": false, "honcho-memory": true };
      },
    };
