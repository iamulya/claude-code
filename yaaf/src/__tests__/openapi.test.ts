...
describe('OpenAPI Naming', () => {
  ...
  describe('generateToolName', () => {
    ...
    it('falls back to method + path', () => {
      ...
      expect(generateToolName(undefined, 'post', '/users/{userId}/orders')).toBe('post_users_by_id_orders')
    })
    ...
    it('truncates long names', () => {
      const longId = 'thisIsAnExtremelyLongOperationIdThatShouldDefinitelyBeTruncatedToSixtyCharacters'
...
describe('OpenAPI Parser', () => {
  it('extracts all operations', () => {
    ...
    expect(operations.map(o => o.operationId)).toEqual([
      'listPets', 'createPet', 'showPetById', 'deletePet',
    ])
  })
  ...
  it('resolves external file $ref (whole file)', () => {
    ...
    const { operations } = parseOpenAPISpec(
      specWithExternalRef as Record<string, unknown>,
      { fileResolver },
    )
  ...
  it('resolves external file $ref with JSON Pointer (file + pointer)', () => {
    ...
    const { operations } = parseOpenAPISpec(
      specWithFileAndPointer as Record<string, unknown>,
      { fileResolver },
    )
  ...
  it('caches external file loads (calls resolver once per file)', () => {
    ...
    const { operations } = parseOpenAPISpec(
      specWithMultipleRefs as Record<string, unknown>,
      { fileResolver },
    )
  ...
  it('throws helpful error for external ref without file resolver', () => {
    ...
    expect(() =>
      parseOpenAPISpec(specWithExternalRef as Record<string, unknown>),
    ).toThrow('no file resolver is configured')
  })
})
...
describe('OpenAPI Schema', () => {
  ...
  it('handles body/param name collisions', () => {
    const op = {
      ...
      parameters: [{ name: 'name', in: 'query' as const, required: false, schema: { type: 'string' } }],
      requestBody: {
        required: true,
...
describe('OpenAPI Auth', () => {
  describe('applyAuth', () => {
    it('applies API key to header', () => {
      ...
      applyAuth(headers, query, { type: 'apiKey', in: 'header', name: 'X-API-Key', value: 'secret' })
...
describe('RestApiTool', () => {
  ...
  it('applies auth to requests', async () => {
    ...
    const fetchHeaders = (fetchSpy.mock.calls[0]![1] as RequestInit).headers as Record<string, string>
...
describe('OpenAPIToolset', () => {
  it('creates tools from a spec object', () => {
    ...
    expect(toolset.tools.map(t => t.name)).toEqual([
      'list_pets', 'create_pet', 'show_pet_by_id', 'delete_pet',
    ])
  })
