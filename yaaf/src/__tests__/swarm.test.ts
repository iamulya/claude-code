...
describe("Workflow: sequential", () => {
  ...
  it("applies custom transform between steps", async () => {
    const pipeline = sequential([mockStep((s) => `Result: ${s}`), mockStep((s) => `Final: ${s}`)], {
...
describe("Workflow: parallel", () => {
  ...
  it("applies custom merge function", async () => {
    const fanOut = parallel([mockStep(() => "fact1"), mockStep(() => "fact2")], {
  ...
  it("respects concurrency limit", async () => {
    ...
    const fanOut = parallel([trackStep("a"), trackStep("b"), trackStep("c"), trackStep("d")], {
      concurrency: 2,
    });
  ...
  it("fail-fast mode aborts on first error", async () => {
    const fanOut = parallel(
      [mockStep(() => "ok"), failingStep("step 2 crashed"), mockStep(() => "ok")],
      { onError: "fail-fast" },
    );
  ...
  it("collect mode replaces failures with error messages", async () => {
    const fanOut = parallel([mockStep(() => "success"), failingStep("boom")], {
      onError: "collect",
    });
...
describe("Workflow: loop", () => {
  ...
  it("applies onIteration transform between iterations", async () => {
    ...
    const loopAgent = loop([mockStep((s) => `processed-${s}`)], {
      maxIterations: 3,
...
describe("Workflow: composition", () => {
  it("nests workflows — sequential inside parallel", async () => {
    const pipelineA = sequential([mockStep((s) => `A1:${s}`), mockStep((s) => `A2:${s}`)]);

    const pipelineB = sequential([mockStep((s) => `B1:${s}`)]);
  ...
  it("nests loop inside sequential", async () => {
    ...
    const pipeline = sequential([mockStep((s) => `drafted:${s}`), refineLoop]);
