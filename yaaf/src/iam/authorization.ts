...
export class RoleStrategy implements AuthorizationStrategy {
  ...
  private readonly compiledRoles: Map<string, {
    allow: RegExp[]
    deny: RegExp[]
  }>
...
export class AttributeStrategy implements AuthorizationStrategy {
  ...
  async evaluate(ctx: AuthorizationContext): Promise<AuthorizationDecision> {
    for (const rule of this.rules) {
      ...
      // Check condition
      const matches = await rule.condition(ctx.user, ctx.arguments)
      if (!matches) continue
...
/**
 * Quick RBAC setup — maps role names to arrays of allowed tool patterns.
 *
 * @example
 * ```ts
 * const auth = rbac({
...
export function rbac(
  ...
  const config: RoleStrategyConfig = {
    roles: Object.fromEntries(
      Object.entries(roles).map(([role, allowed]) => [role, { allow: allowed }]),
    ),
