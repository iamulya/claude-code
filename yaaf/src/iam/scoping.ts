...
export class AttributeScopeStrategy implements DataScopeStrategy {
  ...
  resolve(ctx: ScopeContext): DataScope {
    ...
    for (const rule of this.rules) {
      ...
      if (rule.description) {
        const desc = typeof rule.description === 'function'
          ? rule.description(ctx.user)
          : rule.description
    ...
    return {
      ...
      description: descriptions.length > 0
        ? descriptions.join('; ')
        : 'No attribute scope rules matched',
    }
  }
}
...
export class HierarchyScopeStrategy implements DataScopeStrategy {
  ...
  private readonly config: Required<Omit<HierarchyScopeConfig, 'bypassRoles'>> & { bypassRoles: Set<string> }
...
export class ResolverScopeStrategy implements DataScopeStrategy {
  ...
  async resolve(ctx: ScopeContext): Promise<DataScope> {
    ...
    if (this.cache) {
      ...
      const ttl = resolved.cacheTTL
        ? resolved.cacheTTL * 1000
        : this.cacheTTL
  ...
  return {
    ...
    async resolve(ctx: ScopeContext): Promise<DataScope> {
      ...
      return {
        ...
        filters: {},
        unrestricted: true,
        description: `No system mapping for tool "${ctx.toolName}"`,
      }
    },
  }
}
