...
export class JwtIdentityProvider implements IdentityProvider {
  ...
  async resolve(request: IncomingRequest): Promise<UserContext | null> {
    ...
    // X-5 fix: if a token IS present, verification failures must NOT be swallowed.
  ...
  private async verifyToken(token: string): Promise<JwtPayload> {
    const verifyOptions: JwtVerifyOptions = {
      algorithms: this.config.algorithms ?? (
        this.config.secret ? ['HS256'] : ['RS256', 'ES256']
      ),
    ...
    if (this.config.jwksUri) {
      return verifyJwtWithJwks(token, this.config.jwksUri, verifyOptions)
    }
...
export class ApiKeyIdentityProvider implements IdentityProvider {
  ...
  private extractKey(request: IncomingRequest): string | null {
    ...
    if (this.config.header) {
      const value = request.headers[this.config.header] ?? request.headers[this.config.header.toLowerCase()]
...
export class OidcIdentityProvider implements IdentityProvider {
  ...
  private async getJwtProvider(): Promise<JwtIdentityProvider> {
    if (this._jwtProvider) return this._jwtProvider

    // Deduplicate concurrent discovery requests
    if (!this._discoveryPromise) {
      this._discoveryPromise = this.discover()
    }
  ...
  async resolve(request: IncomingRequest): Promise<UserContext | null> {
    ...
    for (const provider of this.providers) {
      try {
        ...
        // X-6 fix: collect errors instead of silently swallowing.
