...
export type Store<T> = {
   ...
   */
  subscribe<S>(selector: (state: T) => S, listener: (selected: S, state: T) => void): () => void;
};
  ...
  function setState(updater: (prev: T) => T): void {
    ...
    for (const sub of subscribers) {
      try {
        ...
        // A crashing subscriber must not abort notification of
        // subsequent subscribers, which would cause silent state divergence
        // across components that never received the update.
        console.error("[yaaf/store] Subscriber threw during setState notification:", err);
      }
    }
  }
