...
export class AutoMemoryExtractor {
  private readonly config: Required<Omit<AutoExtractorConfig, "onExtracted" | "onError">> &
    Pick<AutoExtractorConfig, "onExtracted" | "onError">;
   ...
   *
   * Loop until no in-flight promise remains. The original
   * implementation awaited only the current inFlightPromise, but
   * runExtractionAsync() launches a trailing run after it completes if
   * pendingArgs is set. Between the old drain() returning and the trailing
   * run being assigned to inFlightPromise, memory writes were silently lost.
   */
  async drain(timeoutMs = 30_000): Promise<void> {
    ...
    while (this.inFlightPromise) {
      ...
      if (remaining <= 0) break; // timeout expired

      await Promise.race([
        this.inFlightPromise,
        new Promise<void>((resolve) => setTimeout(resolve, remaining)),
      ]);
      // After awaiting, re-check: the trailing run may have set a new inFlightPromise.
    }
  }
