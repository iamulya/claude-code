...
export class TeamMemory {
  ...
  async save(
    ...
    // For team-scope entries use a deterministic filename derived
    // from the key rather than a random UUID. This ensures that concurrent
    // save() calls for the same key both write to the same file path, so the
    // OS provides last-writer-wins atomic overwrite semantics instead of
    // creating two divergent copies (split-brain state).
    // Private-scope entries retain random UUIDs for uniqueness.
    const existing = await this.findByKey(data.key, scope);
    const now = new Date().toISOString();

    let entryId: string;
    if (scope === "team") {
      // Deterministic id from key — ensures idempotent file path across agents.
      entryId = slugifyKey(data.key);
    } else {
      entryId = existing?.id ?? crypto.randomUUID().slice(0, 12);
    }
    ...
    await fsp.writeFile(path.join(dir, `${entry.id}.json`), JSON.stringify(entry, null, 2), "utf8");
...
/**
 * Derive a stable filesystem-safe id from a team-memory key.
 * Two agents writing the same key will produce the same filename, giving
 * OS-level last-writer-wins atomic overwrite instead of split-brain.
 */
