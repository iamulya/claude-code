...
function resolvePointer(doc: Record<string, unknown>, pointer: string): unknown {
  ...
  for (const segment of path) {
    ...
    if (current === null || typeof current !== "object") {
      throw new Error(
        `Cannot resolve $ref pointer "${pointer}": path segment "${key}" hit non-object`,
      );
    }
...
export function parseOpenAPISpec(
  ...
  for (const [name, scheme] of Object.entries(rawSchemes)) {
    const resolved = resolveRefs(scheme, spec, new Set(), fileResolver, fileCache) as Record<
      string,
      unknown
    >;
  ...
  const globalSecurityNames = globalSecurity ? globalSecurity.flatMap((s) => Object.keys(s)) : [];
  ...
  for (const [pathStr, pathItem] of Object.entries(paths)) {
    ...
    for (const method of HTTP_METHODS) {
      ...
      const resolved = resolveRefs(operation, spec, new Set(), fileResolver, fileCache) as Record<
        string,
        unknown
      >;
