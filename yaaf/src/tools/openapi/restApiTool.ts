...
export function createRestApiTool(
  ...
  const toolName = nameOverride ?? generateToolName(
  ...
  const description = operation.summary || operation.description || `${operation.method.toUpperCase()} ${operation.path}`
    ...
    async call(
      ...
      try {
        ...
        return {
          data: {
            error: true,
            message: isAbort ? 'Request timed out or was cancelled' : `Connection failed: ${message}`,
            aborted: isAbort,
          },
        }
      }
