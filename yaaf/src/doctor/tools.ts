...
function runCmd(cmd: string, cwd: string): { stdout: string; stderr: string; exitCode: number } {
  try {
...
export function createDoctorTools(projectRoot: string): Tool[] {
  const readFileTool = buildTool<ReadFileInput>({
    ...
    prompt: () => "Read the contents of a file in the project. Supports optional line ranges.",
  });
  ...
  const grepSearchTool = buildTool<GrepSearchInput>({
    ...
    async call({ query, path: searchPath, includes = "*.ts", case_insensitive }) {
      const searchDir = searchPath ? safePath(projectRoot, searchPath) : projectRoot;
  ...
  const runTestsTool = buildTool<RunTestsInput>({
    ...
    maxResultChars: 100_000,
    describe: ({ filter }) => `Run tests${filter ? ` (filter: ${filter})` : ""}`,
    async call({ filter }) {
      const filterArg = filter ? ` -- --grep "${filter}"` : "";
      const { stdout, stderr, exitCode } = runCmd(`npm test${filterArg} 2>&1`, projectRoot);
      const output = (stdout + stderr).trim();
  ...
  return [readFileTool, grepSearchTool, listDirTool, runTscTool, runTestsTool, getStructureTool];
}
