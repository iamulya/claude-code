...
class ErrorTracker {
  ...
  get healthy(): boolean {
    return this.compileErrorCount === 0 && this.testFailureCount === 0;
  }
}
...
export class YaafDoctor {
  ...
  constructor(config: YaafDoctorConfig = {}) {
    ...
    const tools = [...createDoctorTools(this.projectRoot), ...(config.extraTools ?? [])];
   ...
   */
  watch(targetAgent: Agent, options: WatchOptions = {}): this {
    ...
    const handlers = {
      ...
      "llm:retry": (data: {
        ...
        if (data.attempt === 1 || data.attempt >= data.maxRetries) {
          const type: DoctorIssue["type"] =
            data.attempt >= data.maxRetries ? "runtime_error" : "pattern_warning";
          buffer.push({
            type,
      ...
      "context:compaction-triggered": (data: {
        tokensBefore: number;
        tokensAfter: number;
        strategy: string;
      }) => {
  ...
  unwatch(targetAgent: Agent): this {
    ...
    const typedHandlers = entry.handlers as Partial<
      Record<keyof RunnerEvents, (data: unknown) => void>
    >;
